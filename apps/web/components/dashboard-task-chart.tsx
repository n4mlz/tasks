...
export function DashboardTaskChart({
  ...
  return (
    <div data-testid="dashboard-task-chart" className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <div data-testid="dashboard-task-chart-frame" className="h-72 min-h-72 min-w-0">
        {isTest ? (
          chart
        ) : (
      ...
      {chartData.length === 0 ? (
        <p className="mt-3 text-sm text-slate-500">この task の履歴はまだありません。</p>
      ) : null}
    </div>
  );
}
